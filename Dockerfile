...
RUN cd /tmp && \
    cp -R /tmp/incubator-pagespeed-ngx-${NGX_PAGESPEED_VERSION}-stable/* /tmp/ngx_pagespeed-${NGX_PAGESPEED_VERSION}-stable/ && \
    curl -L http://nginx.org/download/nginx-${NGINX_VERSION}.tar.gz | tar -zx && \
    cd /tmp/nginx-${NGINX_VERSION} && \
    LD_LIBRARY_PATH=/tmp/modpagespeed-${PAGESPEED_VERSION}/usr/lib && \
    ./configure \
